 ...
 * fj-core/src/test/resources/core/validator/validator-catalog-test.xml And
 * sample usage in junit test :
...
public class ValidatorCatalog implements Serializable {
		...
		try {
			...
			for (int k = 0; k < tagsValidator.getLength(); k++) {
				...
				if (StringUtils.isEmpty(type)) {
					...
					String parent = current.getAttribute(ATT_PARENT);
	...
	public boolean validate(String validatorId, ValidatorResult result, Locale l, String fieldId, String value,
			String label, Properties params) throws Exception {
