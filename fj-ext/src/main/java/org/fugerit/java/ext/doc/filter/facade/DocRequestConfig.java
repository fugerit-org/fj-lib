...
import org.fugerit.java.ext.doc.filter.DefaultDocHandler;
...
public class DocRequestConfig extends BasicLogObject {
	...
	public void configure( Element root, ConfigContext context ) {
		...
		try {
			...
			// type handlers
			try {
				this.addTypeHandlers(searchDOM, DOMIO.loadDOMDoc( DocRequestFacade.class.getResourceAsStream("/org/fugerit/java/ext/doc/res/type-handler-default.xml") ).getDocumentElement() );
			} catch (Exception e1) {
